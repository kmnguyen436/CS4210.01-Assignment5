#-------------------------------------------------------------------------
# AUTHOR: Kayla Nguyen
# FILENAME: kaylanguyen_assignment5q5.py
# SPECIFICATION: Finds strong rules related to supermarket products.
...
for index, rule in rules.iterrows():
    items_str = ''
    for item in list(rule['antecedents'])[0:]:
        items_str += item + ', '
    items_str = items_str[:-2] + ' -> '
    for item in list(rule["consequents"])[0:]:
        items_str += item + ', '
    items_str = items_str[:-2]
    print(items_str)
